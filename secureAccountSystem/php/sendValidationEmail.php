				...
				if($user['verified'] === 0) {
					if($user['COUNT(requests.id)'] <= MAX_EMAIL_VERIFICATION_REQUESTS_PER_DAY) {
						...
						if($requestID !== -1) {
							...
							$message = '<a href="'. VALIDATE_EMAIL_ENDPOINT . '.php?id=' . $requestID . '&hash=' . urlSafeEncode($verifyCode). '" />Click this link to verify your email</a>';
							if(sendEmail($email, $user['name'], 'Email Verification', $message)) {
								return 0;
							}
							else {
